...
import 'package:flutter_ai_app/views/ai_bot/create_bot_screen.dart';
import 'package:flutter_ai_app/views/ai_bot/edit_bot_screen.dart';
import 'package:flutter_ai_app/views/ai_bot/search_group.dart';
...
class _AIBotState extends State<AIBotView>{
  ...
  void initState() {
    ...
    updateSearchCallback = (String keyword){
      if(keyword == ""){
        ...
        List<AiBot> result = BotManager().searchAiBotWithKeyword(keyword, actualData);
    ...
    return Scaffold(
                          ...
                          ListTile(
                            ...
                            trailing: DropdownButton<String>(
                              ...
                              onChanged: (value){
                                ...
                                switch(value){
                                  case "delete":{
                                    handleDeleteAssistant(lstData[index].id, lstData[index].assistantName);
                                    break;
                                  }
                                  case "edit": {
                                    handleEditAssistant(lstData[index]);
                                    break;
                                  }
                                  default:{
                                    break;
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async{
          Navigator.push(
            context, 
            MaterialPageRoute(builder: (context) => CreateBotScreen(onUpdate: updateDataCallback)),
          );
        },
        child: const Icon(
          Icons.add,
  ...
  void handleEditAssistant(AiBot input) async{
    Navigator.push(context, MaterialPageRoute(builder: (_) => EditBotScreen(data: input, onUpdate: updateEditCallback,)));
  }
